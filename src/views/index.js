console.log("Index.js est en train de démarrer...");
import { embed } from "../utils/ai.js";

const BANQUE_NIVEAUX = Array.isArray(window.BANQUE_NIVEAUX)
...
let motsProchesDecouverts = new Set();

let motsProchesMap = new Map();
...
function chargerNiveau() {
  ...
  document.getElementById("levelIndicator").textContent =
    `Niveau ${niveauActuelIdx + 1} / ${BANQUE_NIVEAUX.length}`;

  const progressionPourcent = Math.round(
    (niveauActuelIdx / BANQUE_NIVEAUX.length) * 100,
  );

  document.getElementById("progressBarFill").style.width =
    `${progressionPourcent}%`;

  document.getElementById("percentIndicator").textContent =
    `${progressionPourcent}%`;
  ...
  motsClesDuTitreSolution = motsSolutionBruts
    .map(normaliserMot)
    .filter((m) => m.length > 0);
  ...
  console.log("Mots du titre chargés :", motsDuTitre);
...
function genererHtmlMots(tableauMots, elementCible) {
  ...
  tableauMots.forEach((item) => {
    if (item.type === "space" || item.type === "punctuation") {
      ...
      } else if (motsProchesDecouverts.has(item.motNettoye)) {
        span.textContent = motsProchesMap.get(item.motNettoye) || "...";
        span.classList.add("warm-word");
        span.dataset.count = item.texteOriginal.length;
...
async function verifierMot(event) {
  ...
  const motSaisi = input.value.trim().toLowerCase();
  ...
  if (niveauDonnees.variantes) {
    ...
    if (niveauDonnees.variantes[motNormalise]) {
      listeMotsAValider = listeMotsAValider.concat(
        niveauDonnees.variantes[motNormalise].map(normaliserMot),
      );
    }
    ...
    for (let cle in niveauDonnees.variantes) {
      if (cle === motNormalise) {
        listeMotsAValider = listeMotsAValider.concat(
          niveauDonnees.variantes[cle].map(normaliserMot),
        );
      }
    }
  }
  ...
  listeMotsAValider.forEach((motDeLaListe) => {
    ...
    if (
      trouveDansTitre ||
      trouveDansTexte ||
      motsClesDuTitreSolution.includes(motDeLaListe)
    ) {
  ...
  motsTrouvesDansCetteSaisie.forEach((mot) => motsTrouvesSet.add(mot));
    ...
    if (occurrencesTotales === 0) {
        let trouveIndice = false;
        for (const targ_mot of motsClesDuTitreSolution) {
            const score = await embed(motNormalise, targ_mot);

            if (score > 0.6) {
                motsProchesMap.set(targ_mot, motNormalise);
                motsProchesDecouverts.add(targ_mot);
                trouveIndice = true;
            }
        }
    if (trouveIndice) rendreLeTexte();
    }
...
function donnerIndices() {
  const indicesDisponibles = niveauDonnees.indices
    .map(normaliserMot)
    .filter((mot) => !motsTrouvesSet.has(mot));
  ...
  historiqueEssais.unshift({
    mot: `💡 Indice: ${motIndice}`,
    occurrences: "Bonus",
  });
...
function verifierVictoire() {
  const aTrouveToutLeTitre = motsClesDuTitreSolution.every((motCle) =>
    motsTrouvesSet.has(motCle),
  );
  ...
  if (aTrouveToutLeTitre && !jeuGagne) {
    ...
    if (isDernierNiveau) {
      ...
      btnSuivant.textContent = "Niveau Suivant";
    }
...
function mettreAJourHistorique() {
  ...
  historiqueEssais.forEach((essai, index) => {
    ...
    const colorClass =
      essai.occurrences > 0 || essai.occurrences === "Bonus"
        ? "text-emerald-600"
        : "text-rose-500";
...
function initialiserJeu() {
  document
    ...
    .addEventListener("submit", (e) => verifierMot(e));
  document.getElementById("btnHint").addEventListener("click", donnerIndices);
  document
    .getElementById("btnNextLevel")
    .addEventListener("click", passerAuNiveauSuivant);
  ...
  if (sauvegardeIdx !== null) {
    ...
    if (
      !Number.isNaN(indexSauvegarde) &&
      indexSauvegarde >= 0 &&
      indexSauvegarde < BANQUE_NIVEAUX.length
    ) {
